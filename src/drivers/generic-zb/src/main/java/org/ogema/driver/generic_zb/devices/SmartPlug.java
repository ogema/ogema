...
import static org.ogema.driver.xbee.Constants.SMART_PLUG_DEVICE_ID;
...
public class SmartPlug extends Generic_ZbDevice implements ResourceValueListener<BooleanResource> {
	...
	private SingleSwitchBox smartPlug;
	// private VoltageResource vRes;
	// private FrequencyResource fRes;
	private String deviceName;
	private List<SampledValueContainer> onOffChannelList;

	public SmartPlug(Generic_ZbDriver driver, ApplicationManager appManager, Generic_ZbConfig config) {
		super(driver, appManager, config);
		unifyResourceName(generic_ZbConfig);
	}

	public SmartPlug(Generic_ZbDriver driver, ApplicationManager appManager, DeviceLocator deviceLocator, String name) {
	...
	private void addMandatoryChannels() {
		...
		offCmdConfig.deviceId = SMART_PLUG_DEVICE_ID;
		...
		onCmdConfig.deviceId = SMART_PLUG_DEVICE_ID;
		...
		toggleCmdConfig.deviceId = SMART_PLUG_DEVICE_ID;
		...
		onOffConfig.deviceId = SMART_PLUG_DEVICE_ID;
		...
		// meterVoltageRMS.deviceId = ChAddrConstants.MAINS_POWER_OUTLET_DEVICE_ID;
		...
		// meterCurrentRMS.deviceId = ChAddrConstants.MAINS_POWER_OUTLET_DEVICE_ID;
		...
		// meterLineFreq.deviceId = ChAddrConstants.MAINS_POWER_OUTLET_DEVICE_ID;
		...
		smartPlug = resourceManager.createResource(generic_ZbConfig.resourceName, SingleSwitchBox.class);
		smartPlug.activate(true);

		// The connection attribute and its children, current, voltage, power, frequency
		ElectricityConnection conn = (ElectricityConnection) smartPlug.electricityConnection().create();
		conn.activate(true);
		...
		//
		// The on/off switch
		onOff = (BooleanResource) smartPlug.onOffSwitch().stateControl().create();
		onOff.activate(true);
		...
		isOn = (BooleanResource) smartPlug.onOffSwitch().stateFeedback().create();
		isOn.activate(true);
	...
	public void updateChannelValue(String chAddr, Value v) {
		if (v == null) {
			Generic_ZbDriver.logger.info(String.format("Smart plug %s channel delivered null value", chAddr));
			return;
		}
		else {
			switch (chAddr) {
