				...
				$.ui.dialog,
				{
					...
					open : function() {
						...
						if (this.options.clickOutside) {
							...
							$(document)
									.on(
											"click.ui.dialogClickOutside"
													+ that.eventNamespace,
											function(event) {
												if ($(event.target)
														.closest(
																$(clickOutsideTriggerEl)).length == 0
														&& $(event.target)
																.closest(
																		$(that.uiDialog)).length == 0) {
													that.close();
												}
											});
						}
...
$(function() {
	...
	$("#dialUpload").dialog(
			{
				...
				buttons : [ {
					...
					click : function() {
						...
						sendPermsToServer(uploadApp, jsonContent,
								"localTempDirectory");
						uploadApp = "";
					}
				} ],
				...
				buttons : [ {
					...
					click : function() {
						...
						if (isReady == true) {
							...
							$.post(
									"/testservice/installedapps?action=setPermission&app="
											+ portletID, {
	...
	function showAppstores() {
		$("#menu").html("");
		$.getJSON("/testservice/appstores", function(json) {
...
function listSysApps() {
	...
	$
			.getJSON(
					"/testservice/installedapps?action=listAll",
					function(json) {
		...
		for (var j = 0; j < policyArr[i].permissions.length; j++) { // Zu jeder
			...
			$("#perm" + i).append(
					"<div class='wrapOnePerm' id='wrapOnePerm" + i + j
							...
							+ policyArr[i].permissions[j].type + "<br></div>");
				...
				$("#polPerm" + i)
						.append(
								"<div class='wrapOnePolPerm' id='wrapPolPerm"
										+ i
										+ j
										+ "'><button class='allowPerButton' onclick='deletePermission("
										+ i
										+ ","
										+ j
										+ ")'></button>Actions<br><input type='checkbox' id='action"
										+ i
										+ j
										+ "' name='"
										+ actionForThis
				...
				$("#polPerm" + i)
						.append(
								"<div class='wrapOnePolPerm' id='wrapPolPerm"
										...
										+ ","
										+ j
										+ ")'></button>Actions: none<br><br><input type='text' value='"
										+ policyArr[i].permissions[j].type
										+ "' id='type"
										+ i
										+ j
...
function getPermContentToSend() {
	...
	for (var x = 0; x < countPolicies; x++) {
		...
		for (var h = 0; h < countPermsInOnePolicy; h++) {
			var currentPerm = thisDia.find("div#wrapPolicy" + x).find(
					".polPermissions").find(".wrapOnePolPerm")[h].id;
			var permType = $("#" + currentPerm).find(".permType").val();
			var permFilter = $("#" + currentPerm).find(".permFilter").val();
			var actionsAsArray = $("#" + currentPerm).find(
			...
			jsonString = '{"mode":"' + mode + '", "permissions":[' + permString
					+ '], "conditions":[';
...
function showAppInfo() {
	...
	if ($(".selectedPortlet")[0]) {
		var portletID = $(".selectedPortlet")[0].id;
		$.getJSON("/testservice/installedapps?action=getInfo&app=" + portletID,
				function(json, xhr) {
...
function updateApp() {
	...
	if ($(".selectedPortlet")[0]) {
		var portletID = $(".selectedPortlet")[0].id;
		$.getJSON("/testservice/installedapps?action=update&app=" + portletID,
				function(json) {
					...
					.jstree(
							{
								"core" : {
									...
									'data' : {
										"url" : "/testservice/installedapps?action=webResources&app="
												+ portletID,
										'data' : function(node) {
											return {
												'id' : node.id,
					...
					.bind(
							...
							function(event, data) {
								for (var x = 0; x < data.node.children.length; x++) {
									var thisNode = $("#treeWebRes")
											.jstree(true).get_node(
													data.node.children[x]);
									...
									if (position != oldName_length - 1) {
										...
										$("#treeWebRes").jstree('rename_node',
												thisNode, newName);
									} else {
										newName = oldName.slice(0, position);
										var newPosition = newName
												.lastIndexOf("/");
										newName = newName
												.slice(newPosition + 1);
										$("#treeWebRes").jstree('rename_node',
...
function deleteApp() {
	if ($(".selectedPortlet")[0]) {
		var portletID = $(".selectedPortlet")[0].id;
		$.getJSON("/testservice/installedapps?action=delete&app=" + portletID,
				function(json) {
				...
				.bind(
						...
						function(event, data) {
							...
							if (!$("#jsTree_Resources").jstree("is_parent",
								...
								$("#dialog_Resources")
										.html("<span><b>Value: </b></span><span>" + currentSelectedResourceNode.original.value + "</span>" + "<br><span><b>Type: </b></span><span>" + currentSelectedResourceNode.original.type + "</span>");
								// open the dialog
	...
	setTimeout(function() {// again File Object
		if (fileChanged == false) {
			if (preAbort == true) {
				...
				client.open("POST", "/testservice/uploadApp");
