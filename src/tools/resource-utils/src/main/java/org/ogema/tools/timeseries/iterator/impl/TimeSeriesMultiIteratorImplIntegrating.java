...
//		this.doIntegrate = doIntegrate;
	...
	protected void init2() {
		for (Map.Entry<Integer, SampledValue> entry : comingValues.entrySet()) {
			...
			final float val = sv.getQuality() == Quality.GOOD ? (doAverage ? sv.getValue().getFloatValue() : 0) : Float.NaN;
			final boolean ok = !Float.isNaN(val);
			nextValuesIntegrated.put(key, new SampledValue(ok ? new FloatValue(val) : FloatValue.NAN, sv.getTimestamp(), 
	...
	private void getNextIntegralValues() {
		...
		for (Map.Entry<Integer, SampledValue> entry: nextValues.entrySet()) {
			...
			if (previous != null) {
				float result = integrate(previous, start, end, next, getInterpolationMode(key));
				final boolean ok = !Float.isNaN(result);
				if (ok) {
					if (doAverage)
						result = result/(end.getTimestamp() - start.getTimestamp());
					else {
						final Float last = lastValidValue.get(key);
						if (last != null)
							result += last;
					}
					nextValuesIntegrated.put(key, new SampledValue(new FloatValue(result), t, Quality.GOOD));
					if (lastValidValue != null)
						lastValidValue.put(key, result);
					// remember bad quality only if no subinterval contains a valid value
				} else {
					nextValuesIntegrated.put(key, new SampledValue(FloatValue.NAN, t, Quality.BAD));
				}
			}
