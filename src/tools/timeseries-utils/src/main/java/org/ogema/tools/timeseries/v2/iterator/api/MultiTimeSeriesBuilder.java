 ...
 * @param <N>
 */
// TODO restrict domain of definition for the resulting time series: start time - end time
public class MultiTimeSeriesBuilder<N> {
	...
	private MultiTimeSeriesBuilder(Collection<ReadOnlyTimeSeries> timeSeries, Class<N> type) {
		...
		if (type != Float.class && type != Double.class && type != Integer.class && type != Long.class && type != Boolean.class && type != String.class)
			throw new IllegalArgumentException(
                    "Illegal type, must be either Float, Double, Boolean, Integer or Long or String, got " + type);
	}
	 ...
	 * sum or average values for float/double time series, {@link #setSum()} or {@link #setAverage()}
	 * can be used instead.
	 * @param function
	 */
	...
	/**
	 * For float/double time series. Note: this resets the function to null.
	 * @throws IllegalStateException
	 * 		if the type is not equal to Float.class or Double.class
	 */
