...
public class TimeSeriesMultiIteratorImplStepSize extends TimeSeriesMultiIteratorImpl {
	...
	final Map<Integer,Double> integrals;
	// complete integrals over the last period
	final Map<Integer,Double> currentIntegrals;
...
//			boolean doAverage, boolean doIntegrate) {
		...
		this.integrals = (doAverage || doIntegrate) ? new HashMap<>(iterators.size()) : null;
		this.currentIntegrals = (doAverage || doIntegrate) ? new HashMap<>(iterators.size()) : null;
		init2();		
	}
...
//				else {
			...
			for (Map.Entry<Integer, SampledValue> entry: comingValues.entrySet()) {
				...
				if (!currentIntegrals.containsKey(idx)) // relevant for initialisation, if first point matches first target time stamp
					currentIntegrals.put(idx, 0D); 
				integrals.put(idx, 0D);
			}
		...
		if (doIntegrate || doAverage) {
			double val;
			for (Map.Entry<Integer, Double> entry: currentIntegrals.entrySet()) {
				val = entry.getValue();
				if (doAverage) {
					val = val/stepSize;
				}
				currentValues.put(entry.getKey(), new SampledValue(new DoubleValue(val), nextTargetTime, Quality.GOOD)); // FIXME quality
			}
		} 
		else
		...
		for (int idx = 0; idx < iterators.size(); idx++) {
			...
			if (doIntegrate) {
				...
				if (last != null) {
					...
					double itgr = integrate(last, last, end, n, mode);
