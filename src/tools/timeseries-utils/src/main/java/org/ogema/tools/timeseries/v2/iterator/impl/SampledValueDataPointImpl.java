...
class SampledValueDataPointImpl extends DataPointImpl<SampledValue> implements SampledValueDataPoint {
	...
	public SampledValue getElement(int idx, InterpolationMode interpolationMode) {
		...
		final double f1 = previous.getValue().getDoubleValue();
		final double f2 = next.getValue().getDoubleValue();
		final double result = (f1 + (f2-f1) * diff1/(diff1+diff2));
		// case interpolationMode linear
		return new SampledValue(new SampledValue(new DoubleValue(result), now, qual));
	}
	...
	@Override
	public double getSum(boolean ignoreMissingPoints, InterpolationMode mode) {
		double value = 0;
		for (int i=0;i<iterator.size();i++) {
			...
			value += sv.getValue().getDoubleValue();
		}
		return value;
	}
