...
//			boolean doAverage, boolean doIntegrate) {
	...
	static final double integrate(final SampledValue previous, final SampledValue start, final SampledValue end, final SampledValue next, final InterpolationMode mode) {
		...
		if (startT == endT)
			return 0;
		if (endT < startT)
			throw new IllegalArgumentException("Interval boundaries interchanged: " + startT + " - " + endT);
		final double p;
		final double n;
		if (mode == null)
		...
		switch (mode) {
		case STEPS:
			return previous.getValue().getDoubleValue() * (endT-startT);
		case LINEAR:
			p = previous.getValue().getDoubleValue();
			n = end.getValue().getDoubleValue();
			return (endT-startT)*(p+(n-p)*(startT+endT-2*previous.getTimestamp())/2/(next.getTimestamp()-previous.getTimestamp()));
		case NEAREST:
			p = previous.getValue().getDoubleValue();
			n = next.getValue().getDoubleValue();
