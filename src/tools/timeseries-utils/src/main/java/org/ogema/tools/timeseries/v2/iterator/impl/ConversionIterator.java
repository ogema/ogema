...
abstract class ConversionIterator implements Iterator<SampledValue> {
	...
	static class GenericSumIterator extends ConversionIterator {
		...
		public SampledValue next() {
			...
			for (int n=0;n<point.inputSize();n++) {
				...
				cnt++;
				result += sv.getValue().getDoubleValue();
			}
			if (cnt == 0)
				result = Double.NaN;
			...
			if (Double.isNaN(result))
				return new SampledValue(DoubleValue.NAN, point.getTimestamp(), Quality.BAD);
			return new SampledValue((result == 0F ? DoubleValue.ZERO : new DoubleValue(result)), point.getTimestamp(), Quality.GOOD);
		}
	...
	static class FunctionIterator<N> extends ConversionIterator {
		...
		public SampledValue next() {
			...
			if (timeSeries.type == Float.class && Float.isNaN((Float) result) || timeSeries.type == Double.class && Double.isNaN((Double) result))
				q = Quality.BAD;
