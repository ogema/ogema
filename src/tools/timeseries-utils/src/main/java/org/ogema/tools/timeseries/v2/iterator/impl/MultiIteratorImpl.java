 ...
 * @param <T>
 */
class MultiIteratorImpl<T extends Comparable<T>> implements MultiIterator<T> {
	...
	public MultiIteratorImpl(final List<Iterator<T>> iterators, final int maxNrHistoricalValues,
		...
		this.historicalValues = (maxNrHistoricalValues != 0 ? new EvictingQueue<DataPointImpl<T>>(maxNrHistoricalValues) : null);
		init();
	}
