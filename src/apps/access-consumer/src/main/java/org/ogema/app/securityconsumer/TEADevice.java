...
public class TEADevice implements org.ogema.core.resourcemanager.ResourceListener {
	private final static Logger logger = LoggerFactory.getLogger(TEADevice.class);

	// use the modbus-tcp driver
	private static final String DRIVER_ID = "modbus";
	...
	private void update(List<SampledValueContainer> channels) {
		...
		for (SampledValueContainer c : channels) {
			...
			//logger.info("TEADevice.update: value=" + value.getValue().getByteArrayValue());
		}
...
//		channels.add(channelAccess.getChannelLocator(CHANNEL_ADDRESS_27D, deviceLocator));
	...
	private void setModel(TEAResource model, SampledValue value) {
		...
		{
			logger.info("TEADevice.setModel: Quality.BAD");
		}
		else if (value.getValue() == null)
		{
			logger.info("TEADevice.setModel: value=<null>");
		}
		else
		{
			int[] array = (int[])value.getValue().getObjectValue();
			logger.info("TEADevice.setModel: value=" + Arrays.toString(array));
		}
	...
	public void resourceChanged(Resource resource) {
		...
		if (resource instanceof IntegerResource) {
			if (resource.isActive() && resource.isWriteable()) {
				try {
					int intValue = ((IntegerResource) resource).getValue();
