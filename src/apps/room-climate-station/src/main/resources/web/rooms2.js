...
function room2() {
    $(document).ready(function() {
        ...
        $("#links").html('<a href="index.html" id="back">Zimmerübersicht</a>'
                + '<a href="room.html' + window.location.search + '"' + 'id="overview">Ansicht wechseln</a>');
...
// calculate color of the angle
    switch (zustand) {
        case 0:
            // no status message available, 3 grey segments, in the middel, right and left
        ...
        case 6:
            ...
            break;
        default:
            // status is yellow or undefined, yellow smily, grey segmente, both arrows to the middel
            angle = 1.94 * Math.PI + (22 - 19) * 0.024 * Math.PI + .5 * Math.PI; //rechte zeiger in der Mitte, linker Zeiger in der mitte
            angle2 = 1.06 * Math.PI - (48 - 26) * 0.12 * Math.PI / 40 + .5 * Math.PI;
            colors[0] = radGradientGrau;
            colors[1] = radGradientGrau;
